...
void checkra1n_A9()
{
    ...
    printf("[*] stage 2, usb setup, sending 0x800 of 'A', sends no data\n");
    D.acquire_device();
    ...
    printf("[*] stage 3, sending payload overwrite2\n");
    sleep_ms(500);
    D.acquire_device();\
    ...
    printf("[*] stage 4, sending payload stage2\n");
    sleep_ms(500);
    D.acquire_device();
    payload_stage2(D, payload);

    printf("[*] stage 5, sending pongoOS\n");
    D.usb_reset();
    D.release_device();
    D.idProduct = 0x1338;
    sleep_ms(5000);
